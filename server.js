...
function publicState(room,viewer){
  ...
  const state={
    ...
    voteHistory:room.voteHistory,voteSubmitted:!spectator&&viewer?room.votes.has(viewer.id):false,voteResults:room.voteResults,voteOutcome:room.phase==='vote_result'&&room.voteResults?{approvals:voteApprovals,rejects:voteRejects,approved:voteApprovals>voteRejects}:null,
...
function newRoom(name,token){const p=makePlayer(name,token),code=makeCode();const room={code,hostId:p.id,players:[p],spectators:[],phase:'lobby',returnedPlayers:new Set(),setup:{preset:'standard',custom:{merlin:true,percival:true,assassin:true,morgana:true,mordred:true,oberon:true}},timerSettings:{...TIMER_DEFAULTS},timer:{phase:null,endsAt:null,pausedRemainingSeconds:null},leaderId:null,selectedTeam:[],questIndex:0,quests:[null,null,null,null,null],proposalNumber:0,rejectionCount:0,votes:new Map(),voteResults:null,voteHistory:[],questVotes:new Map(),confirmations:new Set(),resultNextPhase:null,winner:null,winnerReason:null,assassinationTargetId:null,log:[],createdAt:Date.now(),updatedAt:Date.now()};rooms.set(code,room);log(room,`${name} が部屋を作成しました。`);return room;}
...
async function handleApi(req,res,url){
  try{
    ...
    if(req.method==='POST'&&url.pathname==='/api/action'){
      ...
      else if(type==='voteTeam'){if(room.phase!=='vote')throw new Error('現在は投票フェーズではありません。');if(room.votes.has(p.id))throw new Error('すでに投票済みです。');room.votes.set(p.id,!!x.approve);if(room.votes.size===room.players.length){const a=[...room.votes.values()].filter(Boolean).length,r=room.players.length-a;room.voteResults=room.players.map(q=>({id:q.id,approve:room.votes.get(q.id)}));room.voteHistory.push({questIndex:room.questIndex,proposalNumber:room.proposalNumber,proposalInQuest:room.voteHistory.filter(h=>h.questIndex===room.questIndex).length+1,leader:{id:room.leaderId,name:room.players.find(q=>q.id===room.leaderId)?.name||''},team:room.selectedTeam.map(id=>({id,name:room.players.find(q=>q.id===id)?.name||''})),votes:room.players.map(q=>({id:q.id,name:q.name,approve:room.votes.get(q.id)})),approvals:a,rejects:r,approved:a>r});if(a>r){log(room,`チームが承認されました（承認 ${a} / 否認 ${r}）。`);beginResultPhase(room,'vote_result','quest');}else{room.rejectionCount++;log(room,`チームは否認されました（承認 ${a} / 否認 ${r}）。`);beginResultPhase(room,'vote_result',room.rejectionCount>=5?'gameover_rejections':'team_after_reject');}}}
